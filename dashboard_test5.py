...
# List of numeric columns for selection (exclude ID and categorical)
...
selected_plot = st.radio("Select Plot Type:", ["Scatter", "Box"])

# Map readmitted to numeric for scatter plot
readmit_map = {'NO':0, '>30':1, '<30':2}
df['readmitted_numeric'] = df['readmitted'].map(readmit_map)

if selected_plot == "Scatter":
    fig_num = px.scatter(
        df,
        x=selected_numeric,
        y='readmitted_numeric',    # use numeric y
        color='readmitted',        # keep category color
        trendline="ols",
        title=f"Scatter Plot of {selected_numeric} vs Readmission",
        labels={selected_numeric:selected_numeric, 'readmitted_numeric':'Readmission Status'}
    )
    ...
    fig_num = px.box(
        df,
        x='readmitted',            # box plot can use categorical y
        y=selected_numeric,
        color='readmitted',
        title=f"Box Plot of {selected_numeric} vs Readmission"
    )
    st.plotly_chart(fig_num, use_container_width=True)
